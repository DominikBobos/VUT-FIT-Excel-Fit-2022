...
Description : Script for mixing pre-recorded messages with speech examples
"""
...
parser.add_argument("--stats", action="store_true",
                    help="show statistics about the wav files from the given directory from arg mpath=PATH")
parser.add_argument("--lt",
                    help="Will use longer recordings than specified in seconds")  # use audio longer than specified in seconds
parser.add_argument("--st",
                    help="Will use shorter recordings than specified in seconds")  # use audio shorter than specified in seconds
...
def get_duration(file):
	...
	"""
    return pydub.AudioSegment.from_wav(file).duration_seconds


def modify(file):
    """
	modifies the given file in speed, volume and repetitions
	:param file: a file to perform modification
	:return: modified file with info about the change
	"""
    mixedFile = pydub.AudioSegment.from_wav(file)
...
def get_type(file):
    """
	to get info about the category of the pre-recorded message
	:param file: pre-recorded message
...
def make_filename(recordFile, messageFile, position):
    """
	labels mixed recording
	:param recordFile: telephone conversation filename
	:param messageFile: mixed pre-recorded message filename
	:param position: start position of the message
	:return:
	"""
...
def mix():
    """
	modifies pre-recorded messages and mixes them to phone calls and labels them
	:return:
	"""
    ...
    if arguments.onlymodify:
        ...
        for file in rec:
            ...
            if arguments.st and int(arguments.st) < get_duration(file):
                continue
            record = pydub.AudioSegment.from_wav(file)
