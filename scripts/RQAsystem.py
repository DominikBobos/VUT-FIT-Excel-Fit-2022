...
Description : This script performs RQA based systems - RQA analysis,
            Creates reference clusters and performs SDTW clustering from RQA list of candidates
"""
import sys
import librosa
import numpy as np
...
def GetFeature(file):
    """
    Get right feature based on extension of the file
    :param file: file
    :return: right feature label based on file extension
    """
    if file[-3:] == 'wav':
        return 'mfcc'
    elif file[-3:] == 'lin':
...
def CreateKnownCluster(files, feature=None, frame_reduction=1):
    """
    Creation of reference clusters
    :param files: list of files
    :param feature: type of feature vector
    :param frame_reduction: optional - it is used to preserve convention of SDTW clusters
...
def RqaCluster(rqa_list_pkl, feature, playback=False, metric='cosine', frame_reduction=None, rqa_list=None):
    """
    SDTW Clustering of list of candidates from RQA analysis
    :param rqa_list_pkl: persisten pkl with rqa list of candidates
    :param feature: type of feature vector
    :param playback: listen to infiltrated false alarms in list of candidates
    :param metric: used distance metric
    :param frame_reduction: size of averaged frames
    :param rqa_list: python list with rqa list of candidates
    :return: SDTW clusters
    """
    try:
    ...
    for idx, audio in enumerate(rqa_list):
        print("VAD processing {}/{}".format(idx + 1, len(rqa_list)))
    ...
    for idx, file in enumerate(rqa_list_clear):
        ...
        if file[0] in [item for sublist in clust_list for subsublist in sublist for item in subsublist]:
            continue
        start_time = time.time()
        new_cluster = []  # filepath, frames to compare, frame_reduction, DTW distance, length of sliced audio
        ...
        for idx_nested, file_nested in enumerate(rqa_list_clear):
            ...
            print("Processing {}[{}] ({}/{}) with {}[{}] ({}/{}). -> Distance={:.4f}".format(
                ...
                parsed_file_nested[0], "0" if len(parsed_file_nested) < 10 else "1:" + parsed_file_nested[3],
                                                                                         idx_nested + 1,
                len(rqa_list_clear), dtw_distance))
        ...
        clust_list.append(sorted(new_cluster, key=lambda x: x[-1]))  # sorted by length
    ...
    for cluster in cluster_with_one_file:
        # comparing clusters of size 1 to another cluster classes - to classify them into existing clusters
        start_time = time.time()
        ...
        for idx_nested, cluster_nested in enumerate(clust_list):
            have_hit = False
            if idx_nested in [index for sublist in cluster_with_one_file for index in
                              sublist]:  # comparing the cluster of size one
                continue
...
def RqaAnalysis(data, rqa_threshold, frame_reduction, feature, reduce_dimension, second_pass):
    """
    RQA analysis
    :param data: list of files of given feature type
    :param rqa_threshold: threshold for making hard decision
    :param frame_reduction: size of averaged frames
    :param feature: type of feature vector
    :param reduce_dimension: to reduce size of phoneme posteriors by 3
    :param second_pass: if the system is performing RQA only, or it is used as first-pass
    :return: list of evaluated files and score
    """
    ...
    for idx, file in enumerate(data[0]):  # rqa analysis first
        ...
        rec_matrix = librosa.segment.recurrence_matrix(reduced_array.T, width=40 // frame_reduction,
                                                       k=reduced_array.shape[0] // 10,
                                                       mode='affinity',
...
def RqaDtw(train=None, test=None, feature='mfcc', frame_reduction=1, reduce_dimension=True, second_pass=False,
           sdtw=False, cluster=False, metric='cosine', known=False, parallelize=[None, None]):
    """
    ...
    :param reduce_dimension: to reduce size of phoneme posteriors by 3
    :param second_pass: perform second pass
    :param sdtw: use SDTW instead of DTW
    :param cluster: perform clustering
    ...
    if not known:
        try:
            rqa_list = ArrayFromFeatures.OpenPickle(
                "evaluations/objects/rqa_list_{}.pkl".format(feature, frame_reduction))
        except:
    ...
    if not known:
        ArrayFromFeatures.SavePickle("evaluations/objects/rqa_list_{}.pkl".format(feature, frame_reduction), rqa_list)
    ...
    if second_pass and cluster and not known:
        ...
        result_list = DTWsystem.SecondPassCluster(data=test, clust_list=clust_list, hit_threshold=hit_threshold,
                                                  ...
                                                  metric=metric, sdtw=sdtw, known=False, parallelize=parallelize)

    if second_pass and cluster and known:
        from main import GetFiles
        cluster_train = GetFiles('sub_eval/eval_goal/', feature)
        clust_list = CreateKnownCluster(cluster_train, feature)  # frame_reduction = frame reduction
        result_list = DTWsystem.SecondPassCluster(data=test, clust_list=clust_list, hit_threshold=hit_threshold,
                                                  frame_reduction=frame_reduction, feature=feature,
                                                  reduce_dimension=reduce_dimension, rqa_time=rqa_time,
                                                  metric=metric, sdtw=sdtw, known=known, parallelize=parallelize)

    return result_list


if __name__ == "__main__":
    # Create clusters when using the RQAsystem.py directly
